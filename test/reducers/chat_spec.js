...
describe('reducer', () => {
    ...
    describe('SET_STATE', () => {
        ...
        it('handles SET_STATE', () => {
            ...
            expect(nextState).to.equal(presetInitialState);
        });
    ...
    describe('SET_MOOD', () => {
        ...
        it('should set the mood to the new value', () => {
            expect(nextState.get('mood')).to.equal(mood);
        });
    ...
    describe('DREW_SENDS_MESSAGE', () => {
        ...
        it('should add the message to the message history', () => {
            expect(nextState.get('messageHistory').size).to.equal(presetInitialState.get('messageHistory').size + 1);
            expect(nextState.get('messageHistory').get(nextState.get('messageHistory').size - 1).get('text')).to.equal(message);
        });

        it('should set the message as being written by Drew', () => {
            expect(nextState.get('messageHistory').get(nextState.get('messageHistory').size - 1).get('drewSaid')).to.be.true;
        });

        it('should generate an id one higher than the highest current id for the added message', () => {
            expect(nextState.get('messageHistory').get(nextState.get('messageHistory').size - 1).get('id')).to.equal(highestMessageID + 1);
        });

        it('should set typing to false in the typing state', () => {
            expect(nextState.get('typingState').get('typing')).to.be.false;
        });

        it('should set running to false in the typing state', () => {
            expect(nextState.get('typingState').get('running')).to.be.false;
        });
    ...
    describe('DREW_STARTS_TYPING', () => {
        ...
        it('should set typing to true in the typing state', () => {
            ...
            expect(nextState.get('typingState').get('typing')).to.be.true;
        });
    ...
    describe('USER_SENDS_MESSAGE', () => {
        ...
        it('should set the message as not being written by Drew', () => {
            expect(nextState.get('messageHistory').get(nextState.get('messageHistory').size - 1).get('drewSaid')).to.be.false;
        });
        ...
        it('should set running to true in the typing config', () => {
            expect(nextState.get('typingState').get('running')).to.be.true;
        });

        it('should set the values of waitTime in the typing config', () => {
            expect(nextState.get('typingState').get('waitTime')).to.equal(drewWaitTime);
        });

        it('should set the values of typeTime in the typing config', () => {
            expect(nextState.get('typingState').get('typeTime')).to.equal(drewTypeTime);
        });
